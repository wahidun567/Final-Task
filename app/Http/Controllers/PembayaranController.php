...
// mengambil data tahun dan nominal dari table spp
        ...
        if (!$pembayaran) {
            DB::transaction(function() use($request, $petugas) {
                foreach ($request->bulan_bayar as $bulan) {   
                    Pembayaran::create([
                        'kode_pembayaran' => 'SPPKU'.Str::upper(Str::random(5)),
            ...
            return redirect('pembayaran/history-pembayaran')->with('success', 'Pembayaran berhasil disimpan!');
        }else{
            return back()
    ...
    public function statusPembayaran(Request $request)
    {
        ...
        return view('pembayaran.status-pembayaran', compact('data'));
    }
